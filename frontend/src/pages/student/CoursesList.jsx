...
const CoursesList = () => {
  ...
  return (
    <>
    <div className='relative md:px-36 px-20 pt-20 text-left'>
      <div className='flex flex-col md:flex-row gap-6 items-start justify-between w-full'>
        <div>
          <h1 className='text-4xl font-semibold text-gray-800'>Course List</h1>
          <p className='text-gray-500'>
            <span onClick={() => navigate('/')}
              className='text-blue-600 cursor-pointer'>Home</span> | <span>Course List</span></p>
        </div>
      ...
      {input && <div className='inline-flex items-center gap-4 px-4 py-2 border mt-8 -mb-8 text-gray-600'>
        <p>{input}</p>
        <img className='cursor-pointer' onClick={() => navigate('/course-list')}
